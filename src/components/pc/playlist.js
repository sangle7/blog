...
export default @observer class Articlelist extends React.Component {
    ...
    render() {
        let _temp = AppState.musicData.slice(0, AppState.musicNumber + 10);
		...
		{playlist}
			...
			<footer style={{'display':_temp.length==AppState.musicData.length?'none':'block'}}onClick={this.handleShowMore.bind(this)} className={style.footer}>More</footer>
		</div>)
    }
}
